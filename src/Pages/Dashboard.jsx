...
<ul className="mt-3 space-y-2">{Array.isArray(products) && products.length>=1 && products?.slice(0,5).map(p => <li key={p.id} className="flex items-center justify-between p-2 rounded hover:bg-gray-50"><div><div className="font-medium">{p.name}</div><div className="text-xs text-gray-500">{p.category} • {p.sku}</div></div><div className="text-sm">₹{p.price}</div></li>)}</ul>
</div>
